...
void GLLightSpotApp::Initialize()
{
    mRotation = 0.0f;
    ...
    glEnable(GL_DEPTH_TEST);
     ...
     {
        #include "shaders/include/0022_cube_v.glsl.in"
     };

    const char* vLightShaderStr=
     {
        #include  "shaders/include/0022_light_v.glsl.in"
     };

     const char* fCubeShaderStr=
     {
        #include "shaders/include/0022_cube_f.glsl.in"
     };
 
    const char* fLightShaderStr=
     {
        #include "shaders/include/0022_light_f.glsl.in"
     };
...
void GLLightSpotApp::Render()
{
    ...
    glUniform3f(glGetUniformLocation(mProgramCube, "light.position"), mLightPosition.x, mLightPosition.y, mLightPosition.z);
    glUniform3f(glGetUniformLocation(mProgramCube, "light.direction"), 0.0f, 0.0f, -1.0f);

    glUniform1f(glGetUniformLocation(mProgramCube, "light.cutOff"),  glm::cos(glm::radians(7.5f)));
    glUniform1f(glGetUniformLocation(mProgramCube, "light.outerCutOff"),  glm::cos(glm::radians(10.5f)));


    glUniform1f(glGetUniformLocation(mProgramCube, "light.constant"), 1.0f);
    glUniform1f(glGetUniformLocation(mProgramCube, "light.linear"), 0.09f);
    glUniform1f(glGetUniformLocation(mProgramCube, "light.quadratic"), 0.032f);
