...
void GLCameraApp::Render()
{
    ...
    float camerX = sin(mRotation) * 10.0f;
    float camerZ = cos(mRotation) * 10.0f;
    glm::vec3 targetP(0.0, 0.0, 0.0);
    glm::vec3 cameraUp(0.0, 1.0, 0.0);
    mView = glm::lookAt(glm::vec3(camerX, 0.0f, camerZ), targetP,cameraUp);

    for (int i=0; i< 10;i++)
    {
        mModel =  glm::translate(glm::mat4(1.0f), mPos[i]);
        mModel = glm::rotate(mModel, mRotation * (i+1), glm::vec3(1.0f,0.5f,0.0f));
